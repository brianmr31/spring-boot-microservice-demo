...
public class CustomerBankController {
    ...
    public CustomerBankEntity getById(@PathVariable String id ) throws BadRequestException { 
        return this.customerBankService.getById(id);
    }

    @GetMapping("/api/v1/customers/norek/{rek}")
    public CustomerBankEntity getByRekening(@PathVariable String rek ) throws BadRequestException {
        return this.customerBankService.getByNorek(rek);
    }

    @PatchMapping("/api/v1/customers")
    public MessageDto patchCustomerBank(@Valid @RequestBody CustomerBankDto data) throws BadRequestException {
